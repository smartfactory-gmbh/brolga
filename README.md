...
3. Run setup command: `docker-compose -f docker-compose.dev.yml exec -it brolga_dev mix setup`
...
After going through the local setup and have the stack up and running, you can run the test cases with the following command:

```bash
docker-compose -f docker-compose.dev.yml exec -it brolga_dev mix test
```
